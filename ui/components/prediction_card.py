...
{signal.reasoning}
    ...
    with col_meta1:
        st.markdown(f"""
            <div class="glass-metric-card">
                <div style="font-size:12px; opacity:0.7; margin-bottom:5px;">Model Type</div>
                <div style="font-size:14px; font-weight:bold; color:#6495ed;">{signal.model_type}</div>
            </div>
        """, unsafe_allow_html=True)
    ...
    with col_meta2:
        st.markdown(f"""
            <div class="glass-metric-card">
                <div style="font-size:12px; opacity:0.7; margin-bottom:5px;">Version</div>
                <div style="font-size:14px; font-weight:bold; color:#6495ed;">{signal.model_version}</div>
            </div>
        """, unsafe_allow_html=True)
    ...
    with col_meta3:
        st.markdown(f"""
            <div class="glass-metric-card">
                <div style="font-size:12px; opacity:0.7; margin-bottom:5px;">Last Trained</div>
                <div style="font-size:14px; font-weight:bold; color:#6495ed;">{signal.last_trained}</div>
            </div>
        """, unsafe_allow_html=True)
    
    st.markdown("<br>", unsafe_allow_html=True)
    
    # Additional Insights with glassmorphism cards
    st.markdown("#### 💡 Additional Insights")
    
    insights_col1, insights_col2, insights_col3 = st.columns(3)
    
    with insights_col1:
        signal_strength = "Strong" if signal.confidence > 80 else "Moderate" if signal.confidence > 60 else "Weak"
        st.markdown(
            f'<div class="glass-metric-card">'
            f'<div style="opacity:0.7; font-size:12px; margin-bottom:8px;">SIGNAL STRENGTH</div>'
            f'<div style="font-size:24px; font-weight:bold; margin-bottom:5px;">{signal_strength}</div>'
            f'<div style="opacity:0.6; font-size:11px;">{signal.confidence:.0f}% confidence</div>'
            f'</div>', 
            unsafe_allow_html=True
        )
    ...
    with insights_col2:
        st.markdown(
            f'<div class="glass-metric-card">'
            f'<div style="opacity:0.7; font-size:12px; margin-bottom:8px;">FACTORS ANALYZED</div>'
            f'<div style="font-size:24px; font-weight:bold; margin-bottom:5px;">{len(signal.key_factors)}</div>'
            f'<div style="opacity:0.6; font-size:11px;">indicators evaluated</div>'
            f'</div>', 
            unsafe_allow_html=True
        )
    
    with insights_col3:
        clarity = "Clear" if signal.action != "HOLD" else "Neutral"
        st.markdown(
            f'<div class="glass-metric-card">'
            f'<div style="opacity:0.7; font-size:12px; margin-bottom:8px;">DECISION CLARITY</div>'
            f'<div style="font-size:24px; font-weight:bold; margin-bottom:5px;">{clarity}</div>'
            f'<div style="opacity:0.6; font-size:11px;">{signal.action}</div>'
            f'</div>', 
            unsafe_allow_html=True
        )
